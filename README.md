...
| 版本 | 特性 |
|------|------|
| **v0.8.3** | **场景自动生成**: 舞台由角色行动驱动, 不预先设计 |
| v0.8.2 | **交互式创作**: 三段交互构建+角色权重+多卷循环+新UI |
| v0.8.1 | **外部记忆系统**: RAG 向量检索(SQLite) + FDR 渐进压缩(章→卷→全书), prompt token 减少 80% |
| v0.8.0 | **惰性构建**: 世界观存纯文本, 首次访问时才解析对应地点, 启动 LLM 调用从 8 次降至 2 次 |(SQLite) + FDR 渐进压缩(章→卷→全书), prompt token 减少 80% |
